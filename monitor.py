...
def get_measures(result):
    ...
    while i < samples:
        ...
        # So you should look at average value
        # or implement a measures interval increasing hack.
        ...
        for line in val:
            ...
            if line:
                ...
                if result["dsk"]["_disk_to_mon"] != name:
                    continue
                val = int(line[12])  # time spent doing I/Os (ms)
                if name not in result["dsk"]:
                    result["dsk"][name] = {}
                if "_val" in result["dsk"][name]:
                    # io_time_spent_delta in ms
                    io_time_spent_delta = val - result["dsk"][name]["_val"]
                    # measures_time_spent_delta in seconds
                    measures_time_spent_delta = (t1 - result["dsk"]["_time"])
                    # utilization in percents
                    utilization = \
                        io_time_spent_delta / measures_time_spent_delta / 10
        ...
        for line in out:
            ...
            if name in result["net"]:
                if "_rx" in result["net"][name]:
                    rx_delta = int(line[1]) - result["net"][name]["_rx"]
                    tx_delta = int(line[9]) - result["net"][name]["_tx"]
                    time_delta = t1 - result["net"]["_time"]

                    if result["net"][name]["_full_duplex"]:
                        result["net"][name]["_activity_perc"] = \
                            (rx_delta + tx_delta)
                    else:
                        result["net"][name]["_activity_perc"] = max(rx_delta,
                                                                    tx_delta)
                    result["net"][name]["_activity_perc"] *= \
                        8 / 1024 / 1024 / time_delta
                    result["net"][name]["_activity_perc"] /= \
                        result["net"][name]["_speed_mbps"] * 100
                    result["net"][name]["_activity_perc"] = \
                        round(result["net"][name]["_activity_perc"], 2)
                result["net"][name]["_rx"] = int(line[1])
                result["net"][name]["_tx"] = int(line[9])
                if "_activity_perc_log" in result["net"][name]:
                    result["net"][name]["_activity_perc_log"].append(
                        result["net"][name]["_activity_perc"])
                    sum_act = sum(result["net"][name]["_activity_perc_log"])
                    len_act = len(result["net"][name]["_activity_perc_log"])
                    result["net"][name]["avg_perc"] = \
                        round(sum_act / len_act, 2)
                    result["net"][name]["min_perc"] = \
                        round(min(result["net"][name]["_activity_perc_log"]),
                              2)
                    result["net"][name]["max_perc"] = \
                        round(max(result["net"][name]["_activity_perc_log"]),
                              2)
                    result["net"][name]["perc95"] = \
                        round(numpy.percentile(
                            result["net"][name]["_activity_perc_log"], 95), 2)
                    result["net"][name]["perc99"] = \
                        round(numpy.percentile(
                            result["net"][name]["_activity_perc_log"], 99), 2)
                    # BTW do not need all of this ^^ because of zabbix has
                    # it's own functional for this
                    # (min, max, avg, may be percentile too)
                else:
                    result["net"][name]["_activity_perc_log"] = []
        result["net"]["_time"] = t1

        # PTP
        out = subprocess.run("sudo journalctl -b --no-pager -u ptp4l -n 50",
        ...
        for l in out[::-1]:
            if "master offset" in l:
                result["ptp"]["master_offset"] = \
                    l.split("master offset")[1].split()[0]
                break
    ...
    while i < samples:
        ...
        for k, v in fresult.items():
            if k[:3] == "net" or k[:3] == "dsk":
                ...
                k = "_".join(k)
            # print(k, v)
            # subprocess.Popen(f"/usr/bin/zabbix_sender -z {zabbix_server} "
            #                   "-s {hostname} -k {k} -o".split() + [str(v)],
            #                   stdout=subprocess.DEVNULL)
            subprocess.Popen(f"/usr/bin/zabbix_sender -z {zabbix_server} "
                             f"-s {hostname} -k {k} -o '{v}' "
        ...
        t2 = time.time()
        subprocess.Popen(f"/usr/bin/zabbix_sender -z {zabbix_server} "
                         f"-s {hostname} -k interval_send -o {t2 - t1} "
                         f">/dev/null", shell=True)
...
def main():
    ...
    out = subprocess.run("dmesg", shell=True, check=True,
                         stdout=subprocess.PIPE,
    ...
    for l in out.split("\n"):
        if "Duplex," in l:
            ...
            # are there any nics with speed counted less than Mbps?
            result["net"][name]["_speed_mbps"] = \
                int(l.split("Mbps")[0].strip().split()[-1])

    # find out what iface is responsible for disered net
    net_to_mon = ".".join(zabbix_server.split(".")[:3]+["255"])
    for iface in result["net"]:
        if (net_to_mon in [x['broadcast']
                           for x in
                           netifaces.ifaddresses(iface)[netifaces.AF_INET]]):
            break
    # python rulez
    result["net"], result["net"][iface] = {}, result["net"][iface]
    ...
    print("Get average time spent: ",
          sum(delays["Get"]) / len(delays["Get"]))
    print("Send average time spent:",
          sum(delays["Send"]) / len(delays["Send"]))
